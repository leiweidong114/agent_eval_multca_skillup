# OpenCode 模型 × 六 Agent HI 在线矩阵摘要

- 生成时间：2026-09-03T03:54:37.427070+00:00
- 模型数：1
- Agent 数：6
- 已完成尝试：6
- 通过：3
- 失败：3
- 单条用户消息：`HI`
- 通过定义：Agent 成功 + trace alias 精确命中成功 SpendLogs + 指定模型匹配 + trace key 删除成功

| Agent | 通过/总数 | 通过率 |
|---|---:|---:|
| claude | 0/1 | 0.00% |
| codebuddy | 1/1 | 100.00% |
| codex | 1/1 | 100.00% |
| justdo | 0/1 | 0.00% |
| openclaw | 0/1 | 0.00% |
| opencode | 1/1 | 100.00% |

## 未通过组合

| Agent | 模型 | 尝试 | 状态 | 核验原因 | 错误摘要 |
|---|---|---:|---|---|---|
| claude | `glm-4.7` | 1 | failed | no_database_interactions |  |
| justdo | `glm-4.7` | 1 | failed | no_database_interactions | openclaw --version failed: exit status 69 |
| openclaw | `glm-4.7` | 1 | failed | no_database_interactions |  |
