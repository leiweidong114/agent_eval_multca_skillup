...
<script setup>
import{computed,onMounted,ref}from'vue';import{useRouter}from'vue-router';import{ElMessage}from'element-plus';import{Search}from'@element-plus/icons-vue';import{fetchExperiments,fetchRuns}from'../api'
const router=useRouter(),rows=ref([]),loading=ref(false),type=ref('all'),keyword=ref('');const types=[{label:'全部',value:'all'},{label:'原理图',value:'schematic'},{label:'题库',value:'question'},{label:'Skill',value:'skill'}]
const runType=r=>r.report?.evaluation_type||((r.report?.skills||[]).includes('schematic-generation')?'schematic':'skill')
...
async function load(){loading.value=true;try{const[a,b]=await Promise.allSettled([fetchRuns(),fetchExperiments()]);const runs=a.status==='fulfilled'?a.value:[],exps=b.status==='fulfilled'?b.value:[];rows.value=[...runs.map(r=>({id:r.run_id,name:r.task_name||r.report?.task_name||'Agent 评测',type:runType(r),agent:r.report?.agent||'-',model:r.report?.model||'-',status:r.report?.status||'completed',score:r.report?.summary?.overall_score??r.report?.overall_score,created_at:r.report?.started_at||r.report?.created_at})),...exps.map(e=>({id:e.id,name:e.name||'题库评测',type:'question',agent:e.track==='model_direct'?'direct':'codex',model:'题库 Provider',status:e.status,score:null,completed:e.completed_jobs,total:e.total_jobs,created_at:e.created_at}))].sort((x,y)=>String(y.created_at||'').localeCompare(String(x.created_at||'')))}catch(e){ElMessage.error(e.message)}finally{loading.value=false}}
function open(r){router.push(`/results/${r.type}/${r.id}`)}function typeName(t){return{schematic:'原理图',question:'题库',skill:'Skill'}[t]}function typeTag(t){return{schematic:'success',question:'warning',skill:'primary'}[t]||'info'}function statusText(s){return{completed:'已完成',running:'运行中',queued:'排队中',failed:'失败',cancelled:'已取消',canceled:'已取消'}[s]||s}function statusType(s){return s==='completed'?'success':s==='failed'?'danger':['queued','running'].includes(s)?'warning':'info'}function scoreText(r){if(r.score!==null&&r.score!==undefined)return Number(r.score)<=1?`${Math.round(r.score*100)}%`:Number(r.score).toFixed(1);if(r.total)return`${r.completed||0}/${r.total}`;return'-'}function formatTime(v){return v?new Date(v).toLocaleString('zh-CN'):'时间未知'}onMounted(load)
</script>
<style scoped>
