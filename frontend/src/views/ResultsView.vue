  ...
  <div class="page-stack">
    ...
    <el-card shadow="never" class="panel">
      ...
      <el-table :data="filtered" v-loading="loading" @row-click="open" row-class-name="clickable-row">
        <el-table-column label="评测任务" min-width="260"><template #default="{row}"><div class="title-cell"><b><i v-if="isActive(row.status)" class="live-pulse"/>{{displayTaskName(row)}}</b><span>{{row.id}}</span></div></template></el-table-column>
        <el-table-column label="启动时间" width="180" sortable><template #default="{row}">{{formatTime(row.started_at)}}</template></el-table-column>
        ...
        <el-table-column prop="user_id" label="评测人工号" min-width="125"><template #default="{row}">{{row.user_id||'未记录'}}</template></el-table-column>
        ...
        <el-table-column label="操作" width="230"><template #default="{row}"><el-button link type="primary" @click.stop="open(row)">{{isActive(row.status)?'实时查看':'查看'}}</el-button><el-button v-if="canPrioritize(row)" link type="warning" @click.stop="prioritize(row)">插队</el-button><el-button v-if="isCancelable(row.status)" link type="danger" @click.stop="cancel(row)">取消评测</el-button></template></el-table-column>
      </el-table>
...
import { cancelBatch, cancelExperiment, cancelJob, fetchBatches, fetchExperiments, fetchJobs, fetchRuns, prioritizeBatch, prioritizeJob } from '../api'
...
const isCancelable=status=>['running','queued'].includes(status)
const canPrioritize=row=>row.type!=='question'&&(row.status==='queued'||(row.type==='batch'&&row.status==='running'))
const runType=r=>(r.skills||r.report?.skills||[]).includes('schematic-pipeline')?'schematic':(r.evaluation_type||r.report?.evaluation_type||'skill')
...
async function load(showSpinner=false){if(showSpinner)loading.value=true;try{const[a,b,c,d]=await Promise.allSettled([fetchRuns({summary_only:true,include_local:true}),fetchExperiments(),fetchBatches(),fetchJobs()]);const runs=a.status==='fulfilled'?a.value:[],exps=b.status==='fulfilled'?b.value:[],batches=c.status==='fulfilled'?c.value:[],jobs=d.status==='fulfilled'?d.value:[];const mappedRuns=runs.map(r=>{const status=r.status||r.report?.status||'completed';return{id:r.run_id,name:r.task_name||r.report?.task_name||'Agent 评测',type:runType(r),user_id:r.user_id||r.report?.user_id,agent:r.agent||r.report?.agent||'-',model:r.provider_model||r.model||r.report?.provider_model||r.report?.model||'-',status,score:status==='completed'?(r.score??r.report?.scores?.overall_score??r.report?.summary?.overall_score??r.report?.overall_score):null,rankable:r.valid_for_ranking!==false,diagnostic:r.diagnostic_only===true,started_at:r.started_at||r.created_at||r.report?.started_at||r.report?.created_at}});const mappedJobs=jobs.map(j=>({id:j.job_id,name:j.task_name||'Agent 评测',type:j.evaluation_type||'skill',user_id:j.user_id,agent:j.agent||'-',model:j.model||'-',status:j.status,progress:j.progress,score:j.score,rankable:j.valid_for_ranking!==false,diagnostic:j.diagnostic_only===true,started_at:j.started_at||j.created_at}));const all=[...batches.map(x=>({id:x.batch_id,name:x.name||'批量对比评测',type:'batch',user_id:x.user_id,agent:`${x.total_jobs||0} 个组合`,model:x.best?.model||'多模型',status:x.status,progress:x.progress,score:x.best?.overall_score,rankable:true,diagnostic:false,completed:x.completed_jobs,total:x.total_jobs,started_at:x.started_at||x.created_at})),...mappedJobs,...mappedRuns,...exps.map(e=>({id:e.id,name:e.name||'题库评测',type:'question',user_id:e.employee_no||e.owner_username||e.user_id||e.owner_user_id,agent:e.track==='model_direct'?'direct':'codex',model:'题库评测',status:e.status,score:null,rankable:true,diagnostic:false,completed:e.completed_jobs,total:e.total_jobs,started_at:e.started_at||e.created_at}))];const unique=new Map();for(const item of all){const current=unique.get(item.id);if(!current||isActive(item.status)||(!isActive(current.status)&&item.score!==undefined))unique.set(item.id,item)}rows.value=[...unique.values()].sort((x,y)=>String(y.started_at||'').localeCompare(String(x.started_at||'')));clearTimeout(timer);if(rows.value.some(r=>isActive(r.status)))timer=setTimeout(()=>load(false),1200)}catch(e){ElMessage.error(e.message)}finally{loading.value=false}}
function open(r){router.push(`/results/${r.type}/${r.id}`)}
async function cancel(row){try{await ElMessageBox.confirm(row.type==='batch'?'将取消批次中所有尚未结束的评测，已完成结果会保留。':'将终止这项评测，已经生成的运行文件会保留。','确认取消评测',{type:'warning',confirmButtonText:'取消评测',cancelButtonText:'继续运行'});if(row.type==='batch')await cancelBatch(row.id);else if(row.type==='question')await cancelExperiment(row.id);else await cancelJob(row.id);ElMessage.success('已提交取消请求');await load(false)}catch(e){if(e!=='cancel'&&e!=='close')ElMessage.error(e.response?.data?.detail||e.message)}}
async function prioritize(row){try{const result=row.type==='batch'?await prioritizeBatch(row.id):await prioritizeJob(row.id);const count=Number(result?.prioritized_jobs);ElMessage.success(row.type==='batch'?`已将 ${count||0} 个等待中的子任务移到队列前面`:'任务已移到等待队列前面');await load(false)}catch(e){ElMessage.error(e.response?.data?.detail||e.message)}}
