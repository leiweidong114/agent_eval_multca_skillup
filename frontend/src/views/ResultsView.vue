  ...
  <div class="page-stack">
    ...
    <el-card shadow="never" class="panel">
      ...
      <el-table :data="filtered" v-loading="loading" @row-click="open" row-class-name="clickable-row">
        <el-table-column label="评测任务" min-width="260"><template #default="{row}"><div class="title-cell"><b><i v-if="isActive(row.status)" class="live-pulse"/>{{row.name}}</b><span>{{row.id}} · {{formatTime(row.created_at)}}</span></div></template></el-table-column>
...
const types=[{label:'全部',value:'all'},{label:'原理图',value:'schematic'},{label:'题库',value:'question'},{label:'Skill',value:'skill'},{label:'批量对比',value:'batch'}]
let timer
...
async function load(showSpinner=false){if(showSpinner)loading.value=true;try{const[a,b,c,d]=await Promise.allSettled([fetchRuns(),fetchExperiments(),fetchBatches(),fetchJobs()]);const runs=a.status==='fulfilled'?a.value:[],exps=b.status==='fulfilled'?b.value:[],batches=c.status==='fulfilled'?c.value:[],jobs=d.status==='fulfilled'?d.value:[];const mappedRuns=runs.map(r=>{const status=r.report?.status||'completed';return{id:r.run_id,name:r.task_name||r.report?.task_name||'Agent 评测',type:runType(r),agent:r.report?.agent||'-',model:r.report?.provider_model||r.report?.model||'-',status,score:status==='completed'?(r.report?.scores?.overall_score??r.report?.summary?.overall_score??r.report?.overall_score):null,created_at:r.report?.started_at||r.report?.created_at}});const mappedJobs=jobs.map(j=>({id:j.job_id,name:j.task_name||'Agent 评测',type:j.evaluation_type||'skill',agent:j.agent||'-',model:j.model||'-',status:j.status,progress:j.progress,score:j.result?.scores?.overall_score,created_at:j.created_at}));const all=[...batches.map(x=>({id:x.batch_id,name:x.name||'批量对比评测',type:'batch',agent:`${x.total_jobs||0} 个组合`,model:x.best?.model||'多模型',status:x.status,progress:x.progress,score:x.best?.overall_score,completed:x.completed_jobs,total:x.total_jobs,created_at:x.created_at})),...mappedJobs,...mappedRuns,...exps.map(e=>({id:e.id,name:e.name||'题库评测',type:'question',agent:e.track==='model_direct'?'direct':'codex',model:'题库评测',status:e.status,score:null,completed:e.completed_jobs,total:e.total_jobs,created_at:e.created_at}))];const unique=new Map();for(const item of all){const current=unique.get(item.id);if(!current||isActive(item.status)||(!isActive(current.status)&&item.score!==undefined))unique.set(item.id,item)}rows.value=[...unique.values()].sort((x,y)=>String(y.created_at||'').localeCompare(String(x.created_at||'')));clearTimeout(timer);if(rows.value.some(r=>isActive(r.status)))timer=setTimeout(()=>load(false),1200)}catch(e){ElMessage.error(e.message)}finally{loading.value=false}}
function open(r){router.push(`/results/${r.type}/${r.id}`)}
function typeName(t){return{schematic:'原理图',question:'题库',skill:'Skill',batch:'批量对比'}[t]}
...
function scoreText(r){if(r.score!==null&&r.score!==undefined)return Number(r.score)<=1?`${Math.round(r.score*100)}%`:Number(r.score).toFixed(1);if(r.total)return`${r.completed||0}/${r.total}`;return'-'}
