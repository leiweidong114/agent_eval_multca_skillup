...
async function load(){loading.value=true;try{const[a,b,c]=await Promise.allSettled([fetchRuns(),fetchExperiments(),fetchBatches()]);const runs=a.status==='fulfilled'?a.value:[],exps=b.status==='fulfilled'?b.value:[],batches=c.status==='fulfilled'?c.value:[];rows.value=[...batches.map(x=>({id:x.batch_id,name:x.name||'批量对比评测',type:'batch',agent:`${x.total_jobs||0} 个组合`,model:x.best?.model||'多模型',status:x.status,score:x.best?.overall_score,completed:x.completed_jobs,total:x.total_jobs,created_at:x.created_at})),...runs.map(r=>{const status=r.report?.status||'completed';return{id:r.run_id,name:r.task_name||r.report?.task_name||'Agent 评测',type:runType(r),agent:r.report?.agent||'-',model:r.report?.provider_model||r.report?.model||'-',status,score:status==='completed'?(r.report?.scores?.overall_score??r.report?.summary?.overall_score??r.report?.overall_score):null,created_at:r.report?.started_at||r.report?.created_at}}),...exps.map(e=>({id:e.id,name:e.name||'题库评测',type:'question',agent:e.track==='model_direct'?'direct':'codex',model:'题库评测',status:e.status,score:null,completed:e.completed_jobs,total:e.total_jobs,created_at:e.created_at}))].sort((x,y)=>String(y.created_at||'').localeCompare(String(x.created_at||'')))}catch(e){ElMessage.error(e.message)}finally{loading.value=false}}
function open(r){router.push(`/results/${r.type}/${r.id}`)}function typeName(t){return{schematic:'原理图',question:'题库',skill:'Skill',batch:'批量对比'}[t]}function typeTag(t){return{schematic:'success',question:'warning',skill:'primary',batch:'danger'}[t]||'info'}function statusText(s){return{completed:'已完成',partial_failed:'部分失败',running:'运行中',queued:'排队中',failed:'失败',cancelled:'已取消',canceled:'已取消'}[s]||s}function statusType(s){return s==='completed'?'success':s==='failed'?'danger':['queued','running','partial_failed'].includes(s)?'warning':'info'}function scoreText(r){if(r.score!==null&&r.score!==undefined)return Number(r.score)<=1?`${Math.round(r.score*100)}%`:Number(r.score).toFixed(1);if(r.total)return`${r.completed||0}/${r.total}`;return'-'}function formatTime(v){return v?new Date(v).toLocaleString('zh-CN'):'时间未知'}onMounted(load)
</script>
<style scoped>
