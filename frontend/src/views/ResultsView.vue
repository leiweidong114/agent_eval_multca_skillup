  ...
  <div class="page-stack">
    ...
    <el-card shadow="never" class="panel">
      ...
      <el-table :data="filtered" v-loading="loading" @row-click="open" row-class-name="clickable-row">
        ...
        <el-table-column label="操作" width="180"><template #default="{row}"><el-button link type="primary" @click.stop="open(row)">{{isActive(row.status)?'实时查看':'查看'}}</el-button><el-button v-if="isCancelable(row.status)" link type="danger" @click.stop="cancel(row)">取消评测</el-button></template></el-table-column>
      </el-table>
...
import { cancelBatch, cancelExperiment, cancelJob, fetchBatches, fetchExperiments, fetchJobs, fetchRuns } from '../api'
