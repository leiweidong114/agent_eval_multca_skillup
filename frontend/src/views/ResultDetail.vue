...
import { fetchExperiment, fetchExperimentComparison, fetchExperimentResults, fetchRun } from '../api'

const route=useRoute(),loading=ref(false),detail=ref(null),results=ref([]),comparison=ref({}),error=ref('')
const typeLabel=computed(()=>({question:'QUESTION BANK',schematic:'SCHEMATIC',skill:'SKILL'}[route.params.type]||'EVALUATION'))
const title=computed(()=>detail.value?.name||detail.value?.task_name||`${({question:'题库',schematic:'原理图',skill:'Skill'}[route.params.type])}评测结果`)
