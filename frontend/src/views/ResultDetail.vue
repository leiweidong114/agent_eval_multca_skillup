  ...
  <div class="page-stack">
    ...
    <template v-else-if="detail">
      ...
      <el-card v-if="!['question','batch'].includes(route.params.type) && !isLive" shadow="never" class="panel interactions-panel">
        <template #header><div class="section-head"><div><b>模型交互记录</b><span>完整用户/模型对话、逐轮 Token、耗时与工具调用；支持内容搜索和翻页</span></div><el-tag effect="plain">{{interactionTotal}} 次模型调用</el-tag></div></template>
        <div class="interaction-toolbar"><el-input v-model="interactionSearch" clearable placeholder="搜索用户内容、模型回复、工具名或请求 ID" @keyup.enter="searchInteractions"><template #append><el-button @click="searchInteractions">搜索</el-button></template></el-input></div>
        <div class="interaction-summary"><div><span>会话开始</span><b>{{formatTime(interactionSummary.started_at)}}</b></div><div><span>总用时</span><b>{{durationText(interactionSummary.duration_ms)}}</b></div><div><span>总 Token</span><b>{{number(interactionSummary.total_tokens)}}</b></div><div><span>工具 / Subagent</span><b>{{number(interactionSummary.tool_call_count)}} / {{number(interactionSummary.subagent_start_count)}}</b></div></div>
        <div v-if="interactionSessions.length" class="interaction-sessions"><article v-for="session in interactionSessions" :key="session.session_id"><b>{{session.session_id}}</b><span>开始 {{formatTime(session.started_at)}} · {{durationText(session.duration_ms)}} · {{number(session.interaction_count)}} 轮 · {{number(session.total_tokens)}} tokens · 工具 {{number(session.tool_call_count)}} / Subagent {{number(session.subagent_start_count)}}</span></article></div>
        <div v-if="interactions.length" class="result-conversations">
          <article v-for="(item,index) in interactions" :key="item.request_id||index" class="result-turn">
            <header><b>第 {{item.turn_index||((interactionPage-1)*interactionPageSize+index+1)}} 轮 · {{item.model_group||item.model||'未知模型'}}</b><el-tag size="small" :type="item.status==='success'?'success':'danger'">{{item.status||'未知'}}</el-tag><span>{{formatTime(item.start_time)}} · {{durationText(item.request_duration_ms)}} · {{number(item.total_tokens)}} tokens · 工具 {{number(item.tool_call_count)}}</span></header>
            <div class="dialogue"><div v-for="(message,mIndex) in requestMessages(item)" :key="`q-${mIndex}`" class="dialogue-message" :class="roleClass(message.role)"><strong>{{roleLabel(message.role)}}</strong><p>{{contentText(message.content)}}</p><div v-for="tool in message.tool_calls||[]" :key="tool.id||tool.function?.name" class="inline-tool">工具调用 · {{tool.function?.name||tool.name}}<pre>{{readable(tool.function?.arguments||tool.arguments)}}</pre></div></div><div v-for="(message,rIndex) in responseMessages(item)" :key="`a-${rIndex}`" class="dialogue-message assistant"><strong>模型</strong><p>{{contentText(message.content)}}</p><div v-for="tool in message.tool_calls||[]" :key="tool.id||tool.function?.name" class="inline-tool">工具调用 · {{tool.function?.name||tool.name}}<pre>{{readable(tool.function?.arguments||tool.arguments)}}</pre></div></div></div>
            <footer>输入 {{number(item.prompt_tokens)}} · 输出 {{number(item.completion_tokens)}} tokens · Session {{item.session_id||'未记录'}} · Request {{item.request_id||'未记录'}}</footer>
            <details><summary>查看完整原始请求 / 响应</summary><div class="interaction-body"><article><span>请求</span><pre>{{pretty(item.proxy_server_request||item.messages||{})}}</pre></article><article><span>响应</span><pre>{{pretty(item.response||{})}}</pre></article></div></details>
          </article>
        </div>
        <el-empty v-else description="本次运行没有匹配的模型交互；请检查数据库轨迹状态或搜索条件"/>
        <el-pagination v-if="interactionTotal>interactionPageSize" class="interaction-pagination" background layout="total, prev, pager, next, sizes" :total="interactionTotal" v-model:current-page="interactionPage" v-model:page-size="interactionPageSize" :page-sizes="[10,20,50,100]" @current-change="loadInteractions" @size-change="changeInteractionPageSize"/>
      </el-card>
...
async function loadInteractions(){try{const response=await fetchRunInteractions(route.params.id,{page:interactionPage.value,page_size:interactionPageSize.value,search:interactionSearch.value.trim()||undefined});interactions.value=response.items||[];interactionTotal.value=Number(response.total||0);interactionSummary.value=response.summary||{};interactionSessions.value=response.sessions||[]}catch{interactions.value=[];interactionTotal.value=0;interactionSessions.value=[]}finally{interactionsLoaded.value=true}}
async function searchInteractions(){interactionPage.value=1;await loadInteractions()}
async function changeInteractionPageSize(){interactionPage.value=1;await loadInteractions()}
...
const roleLabel=role=>({user:'用户',assistant:'模型',system:'系统',tool:'工具'}[role]||role||'消息')
const roleClass=role=>['user','assistant','system','tool'].includes(role)?role:'system'
