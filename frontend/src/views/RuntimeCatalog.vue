  ...
  <div class="page-stack">
    ...
    <el-card shadow="never" class="panel">
      ...
      <el-table :data="sortedAgents" v-loading="loading" max-height="560">
        ...
        <el-table-column label="Agent 路径" min-width="360"><template #default="{row}"><code>{{row.detected_executable||row.default_command}}</code></template></el-table-column>
        ...
        <el-table-column label="操作" width="130"><template #default="{row}"><el-button type="primary" link :loading="testingAgent===row.agent" :disabled="!row.detected_executable" @click="testAgent(row)">测试可用性</el-button></template></el-table-column>
      </el-table>
    </el-card>
...
import { fetchAgents, fetchDatabaseHealth, fetchModels, testAgentAvailability, testAllModelAvailability, testModelAvailability } from '../api'

const agents=ref([]),modelData=ref({models:[],errors:[]}),database=ref({}),keyword=ref(''),loading=ref(false)
const agentTests=ref({}),testingAgent=ref(''),testingModels=ref(false),testingModel=ref('')
...
const sortedAgents=computed(()=>detectedAgents.value.slice().sort((a,b)=>a.agent.localeCompare(b.agent)))
...
async function load(){loading.value=true;const result=await Promise.allSettled([fetchAgents(),fetchModels(),fetchDatabaseHealth()]);agents.value=result[0].status==='fulfilled'?result[0].value:[];modelData.value=result[1].status==='fulfilled'?result[1].value:{models:[],errors:[{error:result[1].reason?.message}]};database.value=result[2].status==='fulfilled'?result[2].value:{};if(result.every(x=>x.status==='rejected'))ElMessage.error('无法读取运行环境');loading.value=false}
onMounted(load)
</script>
