...
onMounted(async()=>{clockTimer=setInterval(()=>{clock.value=Date.now()},1000);const savedJobId=sessionStorage.getItem(testJobStorageKey);if(savedJobId){testingJudge.value=true;testVisible.value=true;pollJudgeTest(savedJobId)}const [filterResult,settingsResult]=await Promise.allSettled([fetchJudgeInteractionFilters(),fetchSettings()]);if(filterResult.status==='fulfilled')filters.value=filterResult.value;else ElMessage.warning(filterResult.reason?.response?.data?.detail||filterResult.reason?.message);if(settingsResult.status==='fulfilled')currentJudgeModel.value=settingsResult.value?.judge_model||'';else ElMessage.warning(settingsResult.reason?.response?.data?.detail||settingsResult.reason?.message);if(contextId.value)search()})
