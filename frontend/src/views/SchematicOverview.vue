<template>
  <div class="page-stack">
    ...
    <el-card shadow="never" class="panel search-panel">
      ...
      <p class="search-note">默认显示最近请求；两个条件同时填写时按交集检索。每页 50 条，可继续加载。原始请求和响应保留数据库中记录的内容并隐藏鉴权字段；未记录的内容无法补回。</p>
    </el-card>
...
async function search(append=false){
  ...
  try{
    const next=await fetchSchematicInteractions({user_id:userId.value.trim()||undefined,session_id:sessionId.value.trim()||undefined,limit:50,offset:append?data.value.next_offset:0})
    if(append){
      ...
      for(const row of next.interactions){
        ...
        const session=sessions.get(id)||{session_id:id,user_id:row.user_id,agent_id:row.agent_id,models:[],interaction_count:0,total_tokens:0}
        session.models=[...new Set([...session.models,row.model].filter(Boolean))]
        session.interaction_count++;session.total_tokens+=Number(row.total_tokens||0)
