<template>
  <el-dialog v-model="visible" v-loading="loading" width="92%" top="4vh" append-to-body destroy-on-close class="interaction-detail-dialog">
    <template #header>
      <div class="dialog-heading">
        <div><span>{{ eyebrow }}</span><b>{{ dialogTitle || `第 ${turnIndex || '—'} 轮交互详情` }}</b><small>{{ item?.request_id || '请求 ID 未记录' }}</small></div>
    ...
    <div v-if="item" class="dialog-body">
      <div class="facts">
        ...
        <div><span>{{ contextLabel }}</span><code>{{ item.session_id || '未记录' }}</code></div>
      ...
      <el-collapse v-model="sections" class="primary-sections">
        ...
        <el-collapse-item name="request-response">
          ...
          <section class="io-block output-block">
            ...
            <el-alert v-if="item.error" class="interaction-error" type="error" :closable="false" title="请求失败"><pre>{{ item.error }}</pre></el-alert>
          </section>
        </el-collapse-item>
      </el-collapse>
    </div>
...
const props=defineProps({modelValue:Boolean,item:{type:Object,default:null},turnIndex:{type:Number,default:0},loading:{type:Boolean,default:false},eyebrow:{type:String,default:'MODEL INTERACTION'},dialogTitle:{type:String,default:''},contextLabel:{type:String,default:'会话'},actorLabelOverride:{type:String,default:''}})
...
const responseMessages=computed(()=>{const response=responsePayload.value;if(Array.isArray(response.choices))return response.choices.map(choice=>choice.message||choice.delta||(choice.text!=null?{role:'assistant',content:choice.text}:null)).filter(Boolean);if(Array.isArray(response.output))return response.output.filter(output=>!['function_call','custom_tool_call'].includes(output.type)).map(output=>({role:output.role||'assistant',content:output.content||output.output_text||output.text||output.summary||output,reasoning_content:output.reasoning_content,_type:output.type}));const topLevel=response.content??response.output_text??response.text??response.generated_text;return topLevel!=null?[{role:'assistant',content:topLevel,reasoning_content:response.reasoning_content||response.reasoning}]:[]})
...
const actorLabel=computed(()=>props.actorLabelOverride||(props.item?.interaction_scope==='subagent'||Number(props.item?.depth)>0?`Subagent · ${props.item?.subagent_name||`L${props.item?.depth||1}`}`:'主 Agent'))
...
.interaction-error{margin-top:12px}.interaction-error pre{margin:6px 0 0;white-space:pre-wrap;overflow-wrap:anywhere;font:12px/1.55 ui-monospace,SFMono-Regular,Consolas,monospace}
</style>
