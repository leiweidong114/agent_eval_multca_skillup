...
def test_project_extension_skill_is_discovered_without_env_path(tmp_path):
    backend = tmp_path / "backend"
    skill = backend / "extensions" / "skills" / "private-block-reader"
    skill.mkdir(parents=True)
    (skill / "SKILL.md").write_text("# Block reader\n", encoding="utf-8")

    assert resolve_external_skill(backend, "private-block-reader") == skill.resolve()
    assert list_external_skills(backend)[0]["source"] == "external"
