...
class EvaluationJobManager:
    ...
    def submit(self, request: dict[str, Any], skill_dir: Path) -> dict[str, Any]:
        task_id = f"{datetime.now():%Y%m%d}-{uuid.uuid4().hex[:4]}"
    ...
    def submit_batch(
        ...
        *,
        ...
        batch_id = f"batch-{datetime.now():%Y%m%d}-{uuid.uuid4().hex[:4]}"
    ...
    def _run(self, job_id: str, request: dict[str, Any], skill_dir: Path, cancel: threading.Event) -> None:
        ...
        try:
            result = run_evaluation(
                ...
                input_files=request.get("input_file_paths") or [],
    ...
    def get_batch(self, batch_id: str) -> dict[str, Any] | None:
        ...
        if jobs and completed == len(jobs):
            ...
            if source.get("cancellation_requested_at"):
            ...
            if any(job.get("status") == "cancelling" for job in jobs):
                batch_status = "cancelling"
            elif any(job.get("status") == "running" for job in jobs):
    ...
    def cancel_batch(self, batch_id: str) -> dict[str, Any] | None:
        ...
        with self._lock:
            ...
            if batch is None:
                return None
            job_ids = list(batch.get("job_ids") or [])
            batch["cancellation_requested_at"] = datetime.now().isoformat()
            self._save_batch(batch)
        for job_id in job_ids:
            self.cancel(job_id)
