...
1、测试当前的基于cc swithc的模型适配器，是否可以稳定支持当前6个agent使用litellm里opencode的全部可用模型，只需要向模型发生HI即可 ， 2、不要只局限使用 `glm-4.7，而是需要①确定所有模型对6个agent都可用，②选择opencode里最便宜的模型完成一次完整的评测任务，评测任务使用我skill里最新的原理图整版生成任务 3、发现的问题要分析清楚原因和解决方案，记录到docs文件夹里，评测完成后统一解决
